...
if (0 === strpos($text, 'http://')) {
   ...
   $text = ltrim($text,"http://");
   
} 
if (0 === strpos($text, 'https://')) {
   
   $text = ltrim($text,"https://");
...
if (preg_match($urlRegex, $text, $match)) {
	...
	$answer = array("title" => $title, "titleEsc" => $title, "url" => $finalLink, "pageUrl" => $finalUrl, "cannonicalUrl" => cannonicalPage($pageUrl), "description" => strip_tags($description), "descriptionEsc" => strip_tags($description), "images" => $images, "video" => $video, "videoIframe" => $videoIframe,"video_id"=>$video_id);
