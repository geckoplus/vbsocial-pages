...
if( !class_exists( 'vb_background_image' ) ) {
    
    class vb_background_image {
        ...
        private $default_labels     = array(
            ...
            'set'       => 'Set as Background Image',
            'remove'    => 'Remove Background Image',
            'use'       => 'Use as Background Image',
        );
        
        private $default_args       = array(
            'id'        => 'vbsocial-background-image',
            'post_type' => 'vbsocial',
        );
        ...
        public function __construct( $args ) {
            ...
            $this->post_meta_key= 'background_id';
        ...
        public function kd_admin_init() {
            wp_enqueue_script(
                    'vb-bg-image',
                    plugins_url( 'vbsocial-pages' ).'/common/js/vb-bg-image.js',
                    'jquery'
            );
        }
        ...
        public function kd_meta_box_output( $image_id = NULL ) {
            ...
            if( $image_id && get_post( $image_id ) ) {
                ...
                $output.= sprintf(
                        '<a href="#" id="remove-%1$s-image" onclick="vb_FeaImgRemove( \'%1$s\', \'%2$s\', \'%3$s\' ); return false;">',
                        $this->id,
                        $this->post_type,
                        $nonce_field
                );
        ...
        public function kd_add_attachment_field( $form_fields, $post ) {
            ...
            $output = sprintf(
                    '<a href="#" id="%1$s-featuredimage" onclick="vb_FeaImgSet( %3$s, \'%1$s\', \'%2$s\', \'%6$s\' ); return false;">%5$s</a>',
                    $this->id,
                    $this->post_type,
                    $post->ID,
                    $this->labels['name'],
...
function kd_mfi_get_featured_image_id( $image_id, $post_type, $post_id = NULL ) {
    return vb_background_image::get_featured_image_id( $image_id, $post_type, $post_id );
}

function kd_mfi_get_featured_image_url( $image_id, $post_type, $size = 'full', $post_id = NULL ) {
    return vb_background_image::get_featured_image_url( $image_id, $post_type, $size, $post_id );
}

function kd_mfi_get_the_featured_image( $image_id, $post_type, $size = 'full', $post_id = NULL ) {
    return vb_background_image::get_the_featured_image( $image_id, $post_type, $size, $post_id );
}

function kd_mfi_the_featured_image( $image_id, $post_type, $size = 'full', $post_id = NULL ) {
    return vb_background_image::the_featured_image( $image_id, $post_type, $size, $post_id );
}
 $args_bg = array(
            	'id' => 'vbsocial-background-image',
            	'post_type' => 'vbsocial',      // Set this to post or page
           		 'labels' => array(
				'name'      => 'Background Image',
				'set'       => 'Set as Background Image',
				'remove'    => 'Remove Background Image',
...
new vb_background_image( $args_bg );
